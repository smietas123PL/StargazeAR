...
const IMPORT_ADAPTER_OPTIONS: { value: string; label: string }[] = [
  {
    value: GEMINI_IMPORT_ADAPTER,
    label: adapterLabels[GEMINI_IMPORT_ADAPTER] ?? "Gemini CLI (local)",
  },
];
  ...
  return (
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        ...
        <div className="divide-y divide-border">
          {agents.map((agent) => {
            const selectedType = adapterOverrides[agent.slug] ?? GEMINI_IMPORT_ADAPTER;
              ...
              <div key={agent.slug}>
                <div className="flex items-center gap-3 px-4 py-2.5 text-sm">
                  ...
                  <select
                    ...
                    disabled={IMPORT_ADAPTER_OPTIONS.length === 1}
                  >
  ...
  function handleAdapterConfigChange(slug: string, patch: Partial<CreateConfigValues>) {
    setAdapterConfigValues((prev) => ({
      ...
      [slug]: {
        ...(prev[slug] ?? {
          ...defaultCreateValues,
          adapterType: adapterOverrides[slug] ?? GEMINI_IMPORT_ADAPTER,
        }),
        ...patch,
      },
    }));
