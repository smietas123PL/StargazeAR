...
import { llmConfigCheck } from "../checks/llm-check.js";
import {
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  if (setupMode === "advanced") {
    ...
    p.log.step(pc.bold("Gemini"));
    llm = await promptLlm();

    if (llm) {
      const s = p.spinner();
      s.start("Checking Gemini CLI...");
      const llmResult = await llmConfigCheck(llm);
      s.stop(llmResult.status === "pass" ? llmResult.message : pc.yellow(llmResult.message));
    }
