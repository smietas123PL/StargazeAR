...
3. **Agent process** — the adapter spawns the agent runtime (e.g. Gemini CLI)
...
Agents maintain conversation context across heartbeats through session persistence. The adapter serializes session state (e.g. a Gemini session ID) after each run and restores it on the next wake. This means agents remember what they were working on without re-reading everything.
