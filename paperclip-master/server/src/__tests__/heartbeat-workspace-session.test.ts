...
import { sessionCodec as geminiSessionCodec } from "@paperclipai/adapter-gemini-local/server";
...
describe("parseSessionCompactionPolicy", () => {
  it("keeps conservative defaults for gemini local", () => {
    expect(parseSessionCompactionPolicy(buildAgent("gemini_local"))).toEqual({
      enabled: true,
      maxSessionRuns: 200,
  ...
  it("uses legacy fallback defaults for non-session adapters", () => {
    expect(parseSessionCompactionPolicy(buildAgent("process"))).toEqual({
      enabled: false,
      maxSessionRuns: 200,
      maxRawInputTokens: 2_000_000,
      maxSessionAgeHours: 72,
